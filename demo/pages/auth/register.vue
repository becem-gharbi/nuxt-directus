<template>
    <NuxtLayout>
        <n-result v-if="success" status="success" title="Done" description="Your account is successfully created">
            <template #footer>
                <NuxtLink to="/auth/login">
                    <n-button>Go back to login</n-button>
                </NuxtLink>
            </template>
        </n-result>
...
async function handleSubmit() {
    ...
    if (error.value) {
        apiErrors.value.emailAlreadyExists = error.value.errors.some(e => e.message === "Field \"email\" has to be unique.")
    }
